...
gInterpreter.Declare("""
      ...
      auto InvariantM3(const float pt, const float eta, const float phi, const float mass, const float pt1, const float eta1, const float phi1, const float mass1, const float pt2, const float eta2, const float $
            auto x = pt*std::cos(phi);
            auto x1 = pt1*std::cos(phi1);
...
for s in samples:
	...
	df[s] = df[s].Define('weightSSOS','weight_aux')
	df_test[s] = df[s]
...
for s in samples:
	...
	### hists definitions
	df[s] = df[s].Define('jet_1_pt','Jet_pt[JetQInd[0]]')
	df[s] = df[s].Define('jet_1_nmu','Jet_nMuons[JetQInd[0]]')
	df[s] = df[s].Define('jet_2_pt','nJetGood>1 ? Jet_pt[JetnotMuonInd] : 0')
	df[s] = df[s].Define('jet_1_eta','Jet_eta[JetQInd[0]]')
	df[s] = df[s].Define('jet_2_eta','nJetGood>1 ? Jet_eta[JetnotMuonInd] : 0')
	df[s] = df[s].Define('jet_2_mass','nJetGood>1 ? Jet_mass[JetnotMuonInd] : 0')
	...
	df[s] = df[s].Define('InvM_2jets','nJetGood>1 ? InvariantM(Jet_pt[JetQInd[0]],Jet_eta[JetQInd[0]],Jet_phi[JetQInd[0]],Jet_mass[JetQInd[0]],Jet_pt[JetnotMuonInd],Jet_eta[JetnotMuonInd],Jet_phi[JetnotMuonInd],Jet_mass[JetnotMuonInd]) : 0')
	...
	df[s] = df[s].Define('deltapt_jet1_jet2','fabs(Jet_pt[JetQInd[0]]-Jet_pt[JetnotMuonInd])')
	...
	df[s] = df[s].Define('pT_sum','pTsum(MuonGoodInd, ElectronGoodInd, JetQInd, JetnotMuonInd, Muon_pt, Muon_eta, Muon_phi, Muon_mass, Electron_pt, Electron_eta, Electron_phi, Electron_mass, MET_pt, MET_phi, Jet_pt, Jet_eta, Jet_phi, Jet_mass)')
	df[s] = df[s].Define('pT_product','pTprod(MuonGoodInd, ElectronGoodInd, JetQInd, JetnotMuonInd, Muon_pt, Muon_eta, Muon_phi, Muon_mass, Electron_pt, Electron_eta, Electron_phi, Electron_mass, MET_pt, MET_phi, Jet_pt, Jet_eta, Jet_phi, Jet_mass)')
	df[s] = df[s].Define('aux_various','variousSUM(MuonGoodInd, ElectronGoodInd, JetQInd, JetnotMuonInd, Muon_pt, Muon_eta, Muon_phi, Muon_mass, Electron_pt, Electron_eta, Electron_phi, Electron_mass, MET_pt, MET_phi, Jet_pt, Jet_eta, Jet_phi, Jet_mass)')
	...
	df[s] = df[s].Define('InvM3_aux','InvMassBot(JetGoodInd, JetBotInd, JetQInd, Jet_pt, Jet_eta, Jet_phi, JetnotMuonInd, Jet_mass)')
...
if mode == "mc":
        for s in samples:
                if (s[0]+s[1]+s[2]+s[3]+s[4]+s[5] == "ttbar2" and s[-6:]=="_charm"):
                        df[s+"_secondjetgluon"] = df[s].Filter('fabs(jet_2_flavourP)==21')
                        df[s+"_secondjetstrange"] = df[s].Filter('fabs(jet_2_flavourP)==3')
                        df[s+"_secondjetelse"] = df[s].Filter('!(fabs(jet_2_flavourP)==21 || fabs(jet_notmuon_flavourP)==3)')
                        ## Samples correction
...
if args.notfull:
	for s in samples:
		if args.ssos : path_hist = '/nfs/cms/vazqueze/ttbaranalisis/hists/wqq/ssos/histwqq_v1SSOS_'+s+'_range_'+str(args.list[0])+'_'+str(args.list[1])+'.root'
		else: path_hist = '/nfs/cms/vazqueze/ttbaranalisis/hists/wqq/histwqq_v1_'+s+'_range_'+str(args.list[0])+'_'+str(args.list[1])+'.root'
        ...
        for s in samples:
                if args.ssos: path_hist = '/nfs/cms/vazqueze/ttbaranalisis/hists/secondjetflavour/wqq/ssos/histwqq_sjf_v1vBWPSSOS_'+s+'.root'
                else: path_hist = '/nfs/cms/vazqueze/ttbaranalisis/hists/secondjetflavour/wqq/histwqq_sjf_v1vBWP_'+s+'.root'
                myfile = TFile( path_hist, 'RECREATE' )
