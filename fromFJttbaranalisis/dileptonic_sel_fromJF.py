...
#if args.process == "allMC": proc = ["ww","wjets_1","wjets_2","wjets_3","wjets_4","wjets_5","wjets_6","wjets_7","wjets_8","zjets_1","zjets_2","zjets_3","zjets_4","zjets_5","zjets_6","zjets_$
#        "ttbar_sl","ttbar_dl","ttbar_dh","zz","wz","st_1","st_2","st_3","st_4"]
elif (args.process == "ww" or args.process == "wjets_1" or args.process == "wjets_2" or args.process == "wjets_3"
        ...
        or args.process == "ttbar_dh" or args.process == "M" or args.process == "E" or args.process == "wjets_4"
        ): proc = [str(args.process)]
...
for s in samples:
	...
	df[s] = df[s].Filter('nMuonGood==1 && nElectronGood==1').Filter('nJetGood>=2')
	### hists definitions
	#df[s] = df[s].Filter('nMuonGood>0 && nElectronGood>0').Filter('!(nMuonGood==1) != !(nElectronGood==1)').Filter('nJetGood >= 4').Filter('SVJetInd.size() >= 1').Filter('SVJetGood')
...
#        if args.type == "mc":
#               df_muon[s] = df_muon[s].Define('weightSSOS_final','weight_aux*btag_MED_sf[JetGoodInd[JetBotInd[0]]]*btag_LOO_sf[JetGoodInd[JetBotInd[1]]]*musf_tight_id[MuonGoodInd[0]]*musf_tight_reliso[MuonGoodI$
#               df_electron[s] = df_electron[s].Define('weightSSOS_final','weight_aux*elesf_wp80iso[ElectronGoodInd[0]]*btag_MED_sf[JetGoodInd[JetBotInd[0]]]*btag_LOO_sf[JetGoodInd[JetBotInd[1]]]')
#        else:
#               df_muon[s] = df_muon[s].Define('weightSSOS_final','weight_aux')
#               df_electron[s] = df_electron[s].Define('weightSSOS_final','weight_aux')
...
for s in samples:
                if args.ssos: path_hist = '/nfs/cms/vazqueze/ttbaranalisis/hists/dilep/ssos/hists_dilep_fromJF_SSOS_'+s+'.root'
                else: path_hist = '/nfs/cms/vazqueze/ttbaranalisis/hists/dilep/hists_dilep_fromJF_'+s+'.root'
