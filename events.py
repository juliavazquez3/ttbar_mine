...
for name in histNames:
  ...
  if (name=="jet_sv_eta_M" or name=="jet_sv_eta_E"):
    #hist_ttbarCB = histT["ttbar_charmbottom"]
    hist_ttbarC = histT["ttbar_charm"]
    hist_backg = histT[samples[0]]
    for s in samplesB:
      hist_backg.Add(histT[s])
    Qsig1 = hist_ttbarC.Integral()
    #Qsig2 = hist_ttbarCB.Integral()
    Qbak = hist_backg.Integral()
    if name[-1]=="E": Qdat = histD["E"].Integral()
    if name[-1]=="M": Qdat = histD["M"].Integral()
    print("Number of ttbar charm mc events for "+name[-1]+" channel is "+str(Qsig1))
    #print("Number of ttbar charm bottom mc events for "+name[-1]+" channel is "+str(Qsig2))
    print("Number of MC background events for "+name[-1]+" channel is "+str(Qbak))
    print("Number of data events for "+name[-1]+" channel is "+str(Qdat))
    print("Ratio signal over background is "+str(Qsig1/Qbak))
    print("Significance is "+str(Qsig1/sqrt(Qbak)))
  ...
  if (name=="InvM_2jets_M" or name=="InvM_2jets_E"):
    ...
    Qsig1 = hist_ttbarC.Integral(bmin,bmax)
    #Qsig2 = hist_ttbarCB.Integral(bmin,bmax)
    Qbak = hist_backg.Integral(bmin,bmax)
    if name[-1]=="E": Qdat = histD["E"].Integral(bmin,bmax)
    if name[-1]=="M": Qdat = histD["M"].Integral(bmin,bmax)
    print("Number of ttbar charm mc events for "+name[-1]+" channel in the (60,100) invariant mass window is "+str(Qsig1))
    #print("Number of ttbar charm bottom mc events for "+name[-1]+" channel in the (60,100) invariant mass window is "+str(Qsig2))
